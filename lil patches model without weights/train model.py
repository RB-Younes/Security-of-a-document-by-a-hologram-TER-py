...
filepath = "lil patches model without weights/best_model.h5"
...
model.save('lil patches model without weights/last_epoch_model.h5')

with open('lil patches model without weights/history.pkl', 'wb') as file:
...
plt.savefig('lil patches model without weights/Learning Curve (Loss).png')
plt.show()
...
ax.set_xlabel('Epoch')
ax.legend(['train', 'val'], loc='best')
plt.savefig('lil patches model without weights/Learning Curve (Accuracy).png')
plt.show()
